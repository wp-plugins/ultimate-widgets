 ...
 * Twitter Widget
*/
class uw_tweets_widget extends WP_Widget {

	function uw_tweets_widget()
	{
		$widget_ops = array(
			'classname' 	=> 'tweets',
			'description' => __( 'Adds support for your tweets.', 'khositeweb' )
		);
		$control_ops = array('id_base' => 'tweets-widget');
		// register the widget
		$this->WP_Widget('tweets-widget', __( 'Widget - Twitter Widget', 'khositeweb' ), $widget_ops, $control_ops);
	}
					...
					<div class="tweets-container" id="tweets_<?php echo esc_attr( $widget_id ); ?>">
						<ul class="uw-ul" id="jtwt">
							...
							<li class="jtwt_tweet">
								...
								?>
								<a href="http://twitter.com/<?php echo esc_attr( $tweet['user']['screen_name'] ); ?>/statuses/<?php echo esc_attr( $tweet['id_str'] ); ?>" class="jtwt_date"><?php echo esc_attr( $timeAgo ); ?></a>
							</li>
							<?php endforeach; ?>
						</ul>
					</div>
	...
	function ago( $time ) {
		$periods = array( __( 'second', 'khositeweb' ), __( 'minute', 'khositeweb' ), __( 'hour', 'khositeweb' ), __( 'day', 'khositeweb' ), __( 'week', 'khositeweb' ), __( 'month', 'khositeweb' ), __( 'year', 'khositeweb' ), __( 'decade', 'khositeweb' ) );
		$lengths = array( '60', '60', '24', '7', '4.35', '12', '10' );
		$now = time();
		$difference = $now - $time;
		$tense = __( 'ago', 'khositeweb' );
	...
	function form($instance)
	{
		...
		<p>
			<label for="<?php echo $this->get_field_id('consumer_key'); ?>"><?php _e('Consumer Key:', 'khositeweb'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('consumer_secret'); ?>"><?php _e('Consumer Secret:', 'khositeweb'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('access_token'); ?>"><?php _e('Access Token:', 'khositeweb'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('access_token_secret'); ?>"><?php _e('Access Token Secret:', 'khositeweb'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('twitter_id'); ?>"><?php _e('Twitter Username:', 'khositeweb'); ?></label>
			...
			<label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('Number of Tweets:', 'khositeweb'); ?></label>
...
add_action('widgets_init', create_function('', 'return register_widget("uw_tweets_widget");'));